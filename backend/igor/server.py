...
class IgorServer:
    ...
    def __init__(self, api=None, config=CONFIG):
        ...
        self.server.set_fn_client_left(self.__unregister_new_client)
    ...
    def __register_new_client(self, client, server):
        print('New client: "' + str(client['id']) + '" registered')
        logging.debug('New client: "' + str(client['id']) + '" registered')
        self.clients[client['id']] = client
        self.sessions[client['id']] = {}

    def __unregister_new_client(self, client, server):
        print('Client: "' + str(client['id']) + '" unregistered')
        ...
        def delete_session(client_id, server):
            ...
            if client == None:
                print('deleting sesssion')
                del self.sessions[client_id]
        timeout = Timer(10.0, delete_session, [client['id'], self])
    ...
    def add_process(self, process_id, thread):
        output = ProcessOutput(process_id, self, self.__remove_process)
